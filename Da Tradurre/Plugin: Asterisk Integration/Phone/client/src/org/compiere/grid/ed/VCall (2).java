...
import org.adempiere.phone.AsteriskManager;
 ...
 */
public class VCall extends JComponent
	implements VEditor, ActionListener, KeyListener
{
	 ...
	 *	Mouse Listener
	 */
	final class VCall_mouseAdapter extends MouseAdapter
	{
		/**
		 *	Constructor
		 *  @param adaptee adaptee
		 */
		VCall_mouseAdapter(VCall adaptee)
		{
			m_adaptee = adaptee;
		}	//	VString_mouseAdapter

		private VCall m_adaptee;
	...
	public VCall ()
	{
		this ("Call", false, false, true, 20, 60);
	}	//	VCall
	 ...
	 */
	public VCall (String columnName, boolean mandatory, boolean isReadOnly, boolean isUpdateable,
		int displayLength, int fieldLength)
	{
		...
		//	***	Button	***
		m_button.setIcon(Env.getImageIcon("Call10.gif"));	//	should be 10
		...
		{
			addMouseListener(new VCall_mouseAdapter(this));
	...
	/**	Logger	*/
	private static CLogger log = CLogger.getCLogger (VCall.class);
	/** Column Name				*/
	...
	/**
	 * 	Action button pressed - show Call base URL
	 */
	private void action_button()
	{
    	String callString = m_text.getText();
		String message = null;
		if (callString != null && callString.length() > 0)
		{
			try
			{
				m_system = MSystem.get(Env.getCtx());
				m_user = MUser.get(Env.getCtx());
				
				String phoneHost = m_system.getSIPHost();
				String phoneUser = m_system.getSIPUser();
				String phonePW = m_system.getSIPPassword();
				String phoneContext = m_system.getSIPContext();
				
				String userChannel = m_user.getSIPChannel();
				
				Integer callPriority = 1;
				Long callTimeout = 30000L;

				AsteriskManager callManager = new AsteriskManager(phoneHost, phoneUser, phonePW);
				
				callManager.initiateCall(userChannel, phoneContext, callString, callPriority, callTimeout);
				return;
			}
			catch (Exception e)
			{
		...
		ADialog.warn(0, this, "Phone number is empty !", message);
	}	//	action button
