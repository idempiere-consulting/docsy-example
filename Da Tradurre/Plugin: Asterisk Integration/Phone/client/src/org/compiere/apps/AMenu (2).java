...
import org.adempiere.apps.NotifyBox;
...
public final class AMenu extends CFrame
	implements ActionListener, PropertyChangeListener, ChangeListener, ManagerEventListener
{
	/**
	...
	public AMenu ()
	{
		...
		try
		{
			...
			ManagerConnectionFactory factory = new ManagerConnectionFactory(phoneHost, phoneUser, phonePW);
...
//	private DecimalFormat	m_memoryFormat = DisplayType.getNumberFormat(DisplayType.Integer);
	...
	public void onManagerEvent(ManagerEvent e) {
		...
		final MUser m_user = MUser.get(Env.getCtx());
		if (ename.equals("DialEvent")) {
			DialEvent de = (DialEvent) e;
			String s_channel = m_user.getSIPChannel();
			if(de.getDestination().startsWith(s_channel)) {
				int[] user_ids = MUser.getAllIDs(MUser.Table_Name, 
						"phone='" + de.getCallerId() + "' or " + 
						"phone2='" + de.getCallerId() + "'", 
						Trx.createTrxName());
				NotifyBox nbox = new NotifyBox();
				if(user_ids.length > 0) {
					MUser caller = MUser.get(Env.getCtx(),user_ids[0]);
//					nbox.show(Env.getImageIcon("AD32.gif"), "Anruf von: " + caller.getName() + "\n" + de.getCallerId());
					nbox.show(Env.getImageIcon("AD32.gif"), caller, de);
				}
				else {
					nbox.show(Env.getImageIcon("AD32.gif"), "Unbekannter Anrufer: " + "\n" + de.getCallerId());
				}
			}
		}
